...
> **更新履歴**
>
> - 2026-08-02: 初版（§1〜§5、§7〜§12）
> - 2026-08-02: §6「追加の疑問：参照点とは買値のことか」を追加。参照点の候補と更新、買値分布の集計効果（資本利得オーバーハング）を追記。あわせて §5.2 に補足を追加。
> - 2026-08-02: 仮説を生む道具を体系的に比較した文書を新設し、相互リンクを追加。本文書が担当するのは価格形成の「評価」段階であり、他の段階は新文書で扱う。
...
上の整理は、やや悲観的すぎました。正確には次のとおりです。

> **出品者IDが必要な特徴量**は作れません。
> しかし、**集計値の「動き方」を使えば、参加者構造を近似する特徴量は識別子なしでも作れます。**

| ゲーム理論由来の特徴量 | 出品者IDが必要か | 現サンプルデータで作れるか |
|---|---|---|
| 出品者数、1人あたり出品数 | 必要 | × |
| 在庫集中度（上位出品者比率） | 必要 | × |
| **在庫集中度の代理**（`listing_count` の分散÷平均） | **不要** | **○** |
| **売り渋りの検出**（成約で説明できない出品数の減少） | **不要** | **○** |
| **大口出品者の存在**（差分の粒度） | **不要** | **○** |
| **イベント反応の大きさ** | **不要** | **○** |

詳しくは、ゲーム理論ドキュメントの **§18「集計データから構造を読む」** と **§19「データモデル設計」** を参照してください。

- [ゲーム理論ドキュメント §18〜§21](./game_theory_for_collector_price_prediction.md)
...
> 「参照点は買値のことか」「参照点が人によって違うと何が起きるか」については、
> **§6 で詳しく扱っています**。特に、参照点は買値に固定されず更新されること、
> および買値の**分布**が供給量を決めることは、特徴量設計に直結します。
...
| `price_percentile_180d`（過去180日の価格分布における現在位置） | 参照点の分布・処分効果の集計（§6.4） |
| `price_vs_adapted_ref`（更新された参照点からの距離） | 参照点の不完全な更新（§6.3） |
...
| `gains_overhang_180d`（含み益を持つ保有者の推定割合、§6.4） | **成約価格の履歴**（現在は `sold_count` のみで `sold_price` がない） |
| 推定含み損益 | 過去の成約価格分布 |
...
## 6. 追加の疑問：参照点とは「買値」のことか

この章は、2026-08-02 の議論で追加しました。

### 6.1 疑問

§5.4.1（参照点依存・損失回避・処分効果）を読んだうえで、次の理解を持ちました。

```text
人は絶対的な価格ではなく、含み益・含み損で行動する（基準からの差で判断する）。

→ この差は人によって違う。
   なぜなら、人によって買った価格（基準点）が違うから。
...
**大筋は正しいです。ただし2点、補強が必要です。**

| 記述 | 判定 |
|---|---|
| 人は絶対的な価格ではなく、基準からの差で判断する | **正しい**。参照点依存そのもの |
| 含み益・含み損で行動する | **正しい**。処分効果の説明として妥当 |
| この差は人によって違う | **正しい** |
| なぜなら、人によって買った価格が違うから | **理由の一部にすぎない**（→ §6.3） |
| （この先の含意） | **「人によって違う」の先に、決定的な話がある**（→ §6.4） |

特に §6.3 は、そのままにしておくと**特徴量設計が行き止まりになる**ため重要です。

---

### 6.3 補強1：参照点＝買値、とは限らない

#### 理由は2つある

```text
(1) 人によって買った価格が違う           ← 認識に含まれている理由
(2) そもそも何を基準にするかが人によって違う ← こちらが抜けている
```

参照点の候補は、§3.5.3 で挙げたとおり複数あります。買値はその1つにすぎません。

#### 実証研究が示していること

| 分かっていること | 内容 |
|---|---|
| **過去の最高値がよく使われる** | 実験では、参加者は買値よりも**過去の最高値**を参照点として使う傾向が最も強いと報告されている |
| **参照点は更新される** | 買値に固定されず、時間とともに現在価格の方向へ動く |
| **更新は不完全** | 完全には現在価格に追いつかない。買値と直近価格の「中間」あたりに落ち着く |
| **更新は非対称** | **利得後の上方更新のほうが、損失後の下方更新より大きい**。つまり、上がったときは新しい高値をすぐ基準にするが、下がったときは古い高値をなかなか手放さない |

#### わかりやすい例

```text
10年前に 500円 で買ったカードが、今 5万円。

このとき、保有者の頭の中は
  「500円が4万9,500円になった」  ← ではない
  「4万8千円まで行ったのに、今4万5千円か」 ← こちらに近い
...
しかし、参照点が買値だけではないと分かれば、**観測可能な参照点**を使えます。

| 参照点 | 直接観測できるか | 対応する特徴量（§5.4.1） |
|---|---|---|
| 買値 | **できない** | （分布としてなら推定可能 → §6.4） |
| **過去の最高値** | **できる** | `price_vs_peak_90d`, `drawdown_from_peak_90d`, `days_since_peak_90d` |
| 発売時価格・定価 | **できる** | `price_vs_initial` |
| 直近の相場 | **できる** | `price_vs_ma60` |
| キリのいい数字 | **できる** | `dist_to_round_up` |
| 同シリーズの他商品 | **できる** | `relative_strength_ip`（§5.4.7） |

> §5.5 で `price_vs_peak_90d` と `days_since_peak_90d` を最優先の6特徴量に入れたのは、この理由です。
> **買値が観測できないから代わりに使う**のではなく、**そもそも過去の最高値のほうがよく使われる参照点だから**です。

#### 参照点の更新を特徴量にする

参照点が「不完全に、非対称に更新される」なら、それ自体を特徴量にできます。

| 特徴量名 | 計算式 | 意味 |
|---|---|---|
| `days_since_peak_90d` | 直近90日の最高値からの経過日数 | 参照点がどれだけ古いか。長いほど参照点が現在価格に近づいている |
| `peak_staleness` | `days_since_peak_90d / 90` | 参照点の陳腐化の度合い |
| `adapted_reference` | `w × peak_90d + (1-w) × price_ma_30`（`w` は 0.5 程度から試す） | 不完全な更新を模した合成参照点 |
| `price_vs_adapted_ref` | `price / adapted_reference` | 合成参照点からの距離 |

**非対称性の検証方法**

```text
上昇局面（高値更新の直後）と下落局面（高値から離れた局面）で、
days_since_peak が出品数に与える影響の大きさを比べる。

  対称なら  → 参照点の更新は非対称ではない（仮説を棄却）
  非対称なら → 下落局面のほうが参照点が古いまま残っている
```

#### もう1点：買っていない人にも参照点がある

「含み益・含み損で行動する」のは**保有者**の話です。しかし、**これから買う人**にも参照点があります。

```text
売り手（保有者）の参照点
  → 買値、過去の最高値
  → 「損を確定したくない」「あのときの高値では売れたのに」

買い手（これから買う人）の参照点
  → 相場、定価、過去の最安値
  → 「高値づかみしたくない」「先週は2万円だったのに」
```

**損失回避は、保有者だけの現象ではありません。** 買い手にとっての「損」は、高く買ってしまうことです。

したがって、売り手側と買い手側は**別の特徴量**になります。

| 立場 | 参照点 | 特徴量 | 価格への効果 |
|---|---|---|---|
| 売り手 | 過去の最高値 | `price_vs_peak_90d`, `drawdown_from_peak_90d` | 高値圏＝供給増、安値圏＝供給減 |
| 買い手 | 直近の相場・過去の最安値 | `price_vs_ma60`, `price_vs_trough_90d` | 相場より高い＝買い控え |
| 両方 | キリのいい数字 | `dist_to_round_up` | キリ番直下で停滞 |

---

### 6.4 補強2：「人によって違う」の先に、決定的な含意がある

ここが最も価値のある部分です。

**「人によって違う」ということは、市場全体では「分布になる」ということです。**

そして、**現在価格がその分布のどこにいるか**が、供給量を決めます。
...
保有者の買値の分布を見ると:
  ...
  5,000円以下で買った人     30%  → 含み益（大）  → 売りたい
  5,000〜10,000円で買った人  50%  → 含み益（小）  → 様子見
  10,000円以上で買った人     20%  → 含み損       → 売らない
```

ここで価格が 1万5千円に上がると、どうなるでしょうか。

```text
現在価格 = 1万5千円

  5,000円以下で買った人     30%  → 含み益（特大） → 売りたい
  5,000〜10,000円で買った人  50%  → 含み益（大）  → 売りたい  ← 増えた
  10,000円以上で買った人     20%  → 含み益（小）  → 様子見    ← 変わった

→ 「売りたい」側に回る保有者が 30% から 80% に増えた
→ 利益確定の出品が増える
→ 供給が増える
→ 上昇が止まる
```
...
```mermaid
flowchart TD
    A["価格が上昇する"] --> B["含み益を持つ保有者の割合が増える"]
    B --> C["利益確定の動機を持つ人が増える"]
    C --> D["出品が増える"]
    D --> E["供給増加により上昇が鈍る"]
    E --> F["上値が重くなる"]

    G["価格が下落する"] --> H["含み損を抱える保有者の割合が増える"]
    H --> I["損を確定したくない人が増える"]
    I --> J["出品が減る"]
    J --> K["供給減少により下落が鈍る"]
    K --> L["下値が堅くなる"]
...
つまり、**個人レベルの「損したくない」が、集計されて市場レベルの価格パターンを作っている**ということです。

#### 株式市場では実証されている

これは思いつきではなく、実証されています。

**Grinblatt & Han (2005), Journal of Financial Economics「Prospect Theory, Mental Accounting, and Momentum」**

```text
市場全体の含み益の総量（capital gains overhang / 資本利得オーバーハング）
を説明変数に入れると、

  過去リターンによるモメンタムの予測力が消える。
```

これは非常に強い結果です。

| 含み益オーバーハングを入れない場合 | 入れた場合 |
|---|---|
| 中期（数か月）でモメンタムが観測される | **中期モメンタムが消える** |
| 「上がったものは上がり続ける」ように見える | 「含み益の分布が動いていただけ」と分かる |

> **つまり、モメンタムの正体は「処分効果を市場全体で集計したもの」だった、という結論です。**

#### これは「集計の壁」を越えた実例である

§5.3.1 で、行動経済学を使う際の最大の壁として **「集計の壁」**（個人のバイアスが市場価格に残るとは限らない）を挙げました。

Grinblatt & Han の結果は、**その壁を越えた側の実例**です。

```text
個人レベル: 「Aさんは含み損だから売らない」
                ↓ 集計
市場レベル: 「保有者の何%が含み損か」が供給量を決める
                ↓
価格パターン: モメンタムとして観測される
```

しかも §5.3.1 で論じたとおり、**コレクター市場は株式市場よりも裁定が効きにくい**（空売り不可、高い取引コスト、在庫コスト）ため、この効果はより強く残ると考える理由があります。

#### 本システムへの応用

買値そのものは観測できませんが、**分布は推定できます**。

```mermaid
flowchart LR
    A["過去の成約価格の履歴<br>transactions テーブル"] --> B["今この商品を持っている人が<br>いくらで買ったかの分布を推定"]
    B --> C["現在価格が分布のどこにいるかを計算"]
    C --> D["含み益を持つ保有者の推定割合<br>= 潜在的な供給圧力"]
```

**推定の考え方（簡易版）**

```text
仮定: 過去に成約した商品は、まだ市場に残っている（＝現在の保有者である）
      成約からの経過時間で重みをつける（古い取引ほど再放出済みの可能性が高い）

推定含み益割合 = 過去N日の成約のうち、成約価格が現在価格より低かった割合
```
...
| 特徴量名 | 計算式 | 意味 | 必要なデータ |
|---|---|---|---|
| `gains_overhang_180d` | 過去180日の成約のうち、成約価格 < 現在価格 だった件数の割合 | 含み益を持つ保有者の推定割合 | 成約価格の履歴 |
| `mean_unrealized_gain_180d` | `(現在価格 - 過去180日の平均成約価格) / 過去180日の平均成約価格` | 平均的な含み益率 | 成約価格の履歴 |
| `overhang_change_30d` | `gains_overhang_180d` の30日前からの変化 | 供給圧力が高まっているか | 成約価格の履歴 |
| `weighted_overhang` | 経過日数で重みづけした含み益割合 | 保有期間を考慮した版 | 成約価格の履歴 + 日付 |

**現在のサンプルデータでは作れません。** `sold_count`（件数）はありますが、`sold_price`（成約価格）がないためです。

これは、ゲーム理論ドキュメント §19.3.3 で `transactions` テーブル（`item_id`, `sold_at`, `sold_price`, `condition`）を挙げた理由の1つです。

> **成約価格を保存するかどうかで、この特徴量群を作れるかが決まります。**
> 保存コストは小さく、効果は大きい部類に入ります。

**近似版（現在のデータで作れるもの）**

成約価格がなくても、価格履歴だけで粗い代理は作れます。

| 特徴量名 | 計算式 | 意味 |
|---|---|---|
| `price_vs_ma180` | `price / rolling_mean(price, 180).shift(1)` | 過去半年の平均価格帯に対する現在位置。買値分布の中心の代理 |
| `price_percentile_180d` | 過去180日の価格分布における現在価格のパーセンタイル | 「過去の価格のうち、現在価格より低かった割合」＝ 含み益割合の粗い代理 |

`price_percentile_180d` は、**取引が価格に対して一様に発生していたと仮定すれば、`gains_overhang_180d` の近似になります**。仮定は粗いですが、現在のデータで今すぐ作れます。

---

### 6.5 特徴量への落とし込み（まとめ）

| 参照点の種類 | 誰の参照点か | 観測できるか | 特徴量 | 現データで作れるか |
|---|---|---|---|---|
| 過去の最高値 | 売り手 | ○ | `price_vs_peak_90d`, `drawdown_from_peak_90d`, `days_since_peak_90d` | **○** |
| 直近の相場 | 買い手 | ○ | `price_vs_ma60` | **○** |
| 過去の最安値 | 買い手 | ○ | `price_vs_trough_90d` | **○** |
| 発売時価格 | 両方 | ○ | `price_vs_initial` | **○** |
| キリのいい数字 | 両方 | ○ | `dist_to_round_up` | ○（ただし架空データでは効かない。§5.4.4） |
| 更新された参照点 | 売り手 | ○ | `price_vs_adapted_ref`, `peak_staleness` | **○** |
| **買値の分布** | 保有者全体 | **推定のみ** | `price_percentile_180d`（近似版） | **○** |
| **買値の分布** | 保有者全体 | **推定のみ** | `gains_overhang_180d`（本命） | **×（成約価格が必要）** |

---

### 6.6 修正した理解

最初の理解を、次のように書き換えると正確になります。

```text
【修正前】
人は絶対的な価格ではなく、含み益・含み損で行動する（基準からの差で判断する）。
→ この差は人によって違う。なぜなら、人によって買った価格（基準点）が違うから。

【修正後】
人は絶対的な価格ではなく、基準点（参照点）からの差で判断する。
保有者は含み益・含み損で行動し、買い手は「高値づかみかどうか」で行動する。

この差が人によって違う理由は2つある。
  (1) 人によって買った価格が違う
  (2) そもそも何を基準にするかが違う
      （買値・過去の最高値・定価・直近相場・キリのいい数字）

      実証研究では、買値よりも「過去の最高値」がよく使われると報告されている。
      さらに参照点は時間とともに更新される。
      その更新は不完全で、かつ非対称
      （利得後の上方更新のほうが、損失後の下方更新より大きい）。

重要な帰結が2つある。

  A. 買値は観測できない。
     したがって特徴量にするなら、観測可能な参照点
     （過去の最高値・定価・移動平均）を使う。
     これは「妥協」ではなく、むしろ実証的に妥当な選択である。

  B. 「人によって違う」＝ 市場全体では分布になる。
     現在価格が保有者の買値分布のどこにいるかが、供給量を決める。
       価格が上がる → 含み益の保有者が増える → 利益確定の供給が増える
       → これが「上値が重い」の正体
     株式市場では実証済み
     （Grinblatt & Han 2005: 含み益の総量がモメンタムの予測力を説明し尽くす）。
```
...
### 7.4 追加の疑問：参照点とは買値のことか（§6）

| 項目 | 内容 |
|---|---|
| 結論 | **大筋は正しいが、2点の補強が必要** |
| 補強1 | **参照点＝買値、とは限らない。** 買値は候補の1つ。実証研究では過去の最高値のほうがよく使われる。さらに参照点は時間とともに更新され、その更新は不完全かつ非対称（利得後の上方更新のほうが大きい） |
| 補強1の帰結 | 買値は観測できない。しかし観測可能な参照点（過去の最高値・定価・移動平均）を使えばよく、これは妥協ではなく実証的に妥当な選択 |
| 見落としやすい点 | **買い手にも参照点がある。** 損失回避は保有者だけの現象ではない。売り手側と買い手側は別の特徴量になる |
| 補強2 | **「人によって違う」＝ 市場全体では分布になる。** 現在価格が保有者の買値分布のどこにいるかが供給量を決める |
| 補強2の帰結 | 価格が上がるほど含み益の保有者が増え、利益確定の供給が増える。これが「上値が重い」の正体 |
| 実証 | Grinblatt & Han (2005): 含み益の総量を説明変数に入れると、株式市場の中期モメンタムの予測力が消える。**モメンタムの正体は処分効果の集計だった** |
| §5.3.1 との関係 | これは「集計の壁」を**越えた側**の実例。コレクター市場は裁定が効きにくいため、効果はより強く残ると考える理由がある |
| 本システムへの応用 | `price_percentile_180d`（現データで可能、近似版）／`gains_overhang_180d`（成約価格の保存が必要、本命） |
...
判断の基準となる点です。人は絶対的な水準ではなく、参照点からの差で価値を評価します。コレクター市場では、過去の高値、買値、定価などが参照点になります。**買値は候補の1つにすぎず、実証研究では過去の最高値のほうがよく使われると報告されています**（§6.3）。

### 参照点の更新（参照点適応）

参照点が買値に固定されず、時間とともに現在価格の方向へ動くことです。更新は**不完全**（完全には追いつかない）で、かつ**非対称**（利得後の上方更新のほうが、損失後の下方更新より大きい）です。

### 資本利得オーバーハング（capital gains overhang）

市場全体で、保有者がどれだけ含み益を抱えているかを表す量です。個人の処分効果を市場全体に集計したものにあたります。Grinblatt & Han (2005) は、この量を説明変数に入れると株式市場の中期モメンタムの予測力が消えることを示しました（§6.4）。

### 上値が重い

価格が上昇するほど含み益を持つ保有者が増え、利益確定の供給が増えるため、上昇が鈍る現象です。参照点依存と処分効果を市場全体で集計した結果として説明できます。
...
- Grinblatt & Han (2005) "Prospect Theory, Mental Accounting, and Momentum", *Journal of Financial Economics*（含み益の総量がモメンタムを説明する）: https://www-2.rotman.utoronto.ca/facbios/file/momentum_JFE.pdf
- Arkes et al. "Reference Point Adaptation: Tests in the Domain of Security Trading"（参照点の更新は不完全かつ非対称）: https://mpra.ub.uni-muenchen.de/4259/1/MPRA_paper_4259.pdf
- Reference Point Adaptation and Disposition Effect（機関投資家データでの検証）: https://ink.library.smu.edu.sg/cgi/viewcontent.cgi?article=4037&context=lkcsb_research
